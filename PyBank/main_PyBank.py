...
with open(csvpath) as csvfile:
    '''Map info in the row to a dictionary'''
    csvreader= csv.DictReader(csvfile, delimiter=',')
    # to avoid appending the first row to the Change list
    first_row = next(csvreader)
    total_months +=1
    total_profitloss += int(first_row["Profit/Losses"])
    previous_PL = int(first_row["Profit/Losses"])
    ...
    for row in csvreader:
        ...
        change_profitloss = int(row["Profit/Losses"]) - previous_PL
        previous_PL = int(row["Profit/Losses"])
...
output_text_file=os.path.join('..', "Resources", 'Budget_Analysis.txt')
